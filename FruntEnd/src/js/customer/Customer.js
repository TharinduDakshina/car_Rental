        ...
        success: function (res) {
            ...
            for (i in values) {
                ...
                $('#tblPayBody').append(`
                    ...
                    <td>${returndate}</td>
                    <td>${status}</td>
                    <td><button id="cstCancelBooking"  style="background-color: #ecde49;border-radius: 2px" class='editRow'>Cancel</button></td>
                    </tr>`)
            }

            bindClickEventCustomerBooking();
        }
    });
}

function updateDriverCustomerPaymentPage(dID) {
    let dId ;
    let name;
    let contact ;
    let nic ;
...
function updateCarCustomerPaymentPage(cId) {
    let adminCarId ;
    let adminCarBrand ;
    let adminCarColor ;
    let adminCarType ;
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.code == 200) {
                ...
                console.log("Cancel car status update");
            }else {
                console.log(res.message);
            }
        },error:function (ob,textStatus,error){
...
function cancelBooking(bookingId) {
    ...
    $.ajax({
        url: "http://localhost:8080/BackEnd_war/booking/"+bookingId,
        method:"GET",
        async: false,
        success:function (res){
            if (res.constructor = 200) {
...
function bindClickEventCustomerBooking() {
    let bookingId;
    let dId;
    let cId;

    $("#tblPayBody>tr").click(function (){
        cId = $(this).children().eq(1).text();
        dId = $(this).children().eq(2).text();
        bookingId = $(this).children().eq(0).text();
    });



    $('#tblPayBody').on('click', "#cstCancelBooking", function () {
        if (confirm("Do you want to Cancel this Order ?") == true) {
            updateDriverCustomerPaymentPage(dId);
            updateCarCustomerPaymentPage(cId)
            cancelBooking(bookingId);
            loadBooking();
            alert(bookingId+"this is booking canceled .");
        }
    });
}
